...
def insertar_pedido(tipo, descripcion, comuna_id, nombre_comprador, email_comprador, celular_comprador):
    try :
        ...
        conn.commit()
        cursor.execute(QUERY_DICT['obtener_ultimo_id_pedido'])
        id_producto = cursor.fetchone()[0]
        return id_producto
    except Exception as e:
        print(e)
...
def insertar_pedido_verdura_fruta(pedido_id, tipo_verdura_fruta_id):
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['insertar_pedido_verdura_fruta'], (pedido_id, tipo_verdura_fruta_id))
        conn.commit()
    except Exception as e:
        print(e)
    finally:
...
def obtener_cantidad_pedidos():
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['obtener_cantidad_pedidos'])
        cantidad = cursor.fetchone()[0]
        return cantidad
    except Exception as e:
        print(e)
...
def obtener_info_pedido(pedido_id):
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['obtener_info_pedido'], (pedido_id,))
...
def obtener_tvf_pedido(pedido_id):
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['obtener_tvf_pedido'], (pedido_id,))
        tvf = cursor.fetchall()
        return tvf
    except Exception as e:
        print(e)
...
def obtener_ultimos_pedidos_info(page):
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['obtener_ultimos_pedidos_info'], ((page-1)*5))
...
def obtener_ultimos_pedidos(page):
    cantidad = obtener_cantidad_pedidos()
    y = pages.products_in_page(page, cantidad)
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['obtener_ultimos_pedidos'], (y[1],y[0],))
        pedidos = cursor.fetchall()
        return pedidos
    except Exception as e:
        print(e)
...
def prod_stats():
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['prod_stats'])
...
def pedido_stats():
    try :
        conn = get_conn()
        cursor = conn.cursor()
        cursor.execute(QUERY_DICT['pedido_stats'])
        stats = cursor.fetchall()
        return stats
    except Exception as e:
        print(e)
